...
namespace PlaylistSaver.Windows.PopupViews.PlaylistItemInfo
{
    public class PlaylistItemInfoViewModel : ViewModelBase
    {
        ...
        public PlaylistItemInfoViewModel(DisplayPlaylistItem displayPlaylist)
        {
            ...
            if (displayPlaylist.RecoveryFailed)
            {
            ...
            else if (displayPlaylist.FoundSnapshotsCount > 0)
            {
                ...
                FoundSnapshotsText = $"{displayPlaylist.FoundSnapshotsCount} snapshot{LocalHelpers.SEnding(displayPlaylist.FoundSnapshotsCount)}\nfound";
                WebArchiveLink = displayPlaylist.WebArchiveLink;

                if (displayPlaylist.WebArchiveSuccesfulRecovery)
                {
                    InfoText = "This data was recovered from web archive and it might be incorrect or glitchy (please report if it is).\nYou can check the page by yourself by clicking this text.";
                    InfoImage = LocalHelpers.GetResourcesBitmapImage("Logos/WebArchive/webArchiveLogo_32px.png");
                }
                else
                {
                    InfoText = "Web archive snapshot of this video was found, but program was unable to parse it (please report this issue).\nYou can check the page by yourself by clicking this text.";
